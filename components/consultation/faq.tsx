...
export const FaqSection = ({ eyebrow, heading, faqs }: FaqSectionProps) => (
  <section className="w-full px-4 pt-[42px] pb-[41px] sm:px-6">
    <div className="mx-auto w-full max-w-215">
      <p className="text-eyebrow text-text-secondary uppercase">{eyebrow}</p>
      <h2 className="font-heading text-text-primary mt-[3px] text-2xl font-bold sm:text-h3">
        {heading}
      </h2>
      ...
      <Accordion.Root
        type="single"
        collapsible
        className="mt-[42px] flex w-full flex-col"
      >
        {faqs.map((faq, index) => (
          <Accordion.Item
            key={faq.question}
          ...
          >
            <Accordion.Header>
              <Accordion.Trigger className="group flex w-full items-start gap-0 pt-[22px] pb-[18px] text-left">
                <span className="text-eyebrow text-text-secondary mt-1.5 w-[41px] shrink-0">
            ...
            <Accordion.Content className="data-[state=closed]:animate-accordion-up data-[state=open]:animate-accordion-down overflow-hidden">
              <p className="text-body-sm text-text-secondary max-w-[68ch] pb-6 pl-[41px]">
                {faq.answer}
              </p>
            </Accordion.Content>
          </Accordion.Item>
